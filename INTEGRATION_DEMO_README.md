...
**不要用"是否经 UNREAD 拉取"来判断**：实时消息也走 NOTICE → SYNC 拉取，两者在拉取路径上无法区分；登录期间到达的实时消息还会混进同一批。

唯一可靠的依据是服务端下发的 `Message.OnlineState`，它来自 msync `Meta` 字段 9 的 JSON 属性（键为 `is_online`）：

| 值 | 含义 |
| --- | --- |
| `online` | 投递时对端在线，即实时消息 |
| `offline` | 离线消息 |
| `unknown`（零值，JSON 中省略） | 服务端未下发该属性 |

该属性依赖服务端开关，未开启时恒为 `unknown`，且服务端本身也不保证精确，因此只能当提示用，不能作为投递语义的强保证。SDK 不会在服务端沉默时替它猜一个默认值。
